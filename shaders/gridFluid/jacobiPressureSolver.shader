...
uint type(int i, int j){return texelFetch(cellType, ivec2(i, j), 0).r;}

void main()
{
    ive2 id = ivec2(gl_GlobalInvocationID.xy);
    ...
    floar pressureC = texelFetch(pressureIn, id, 0).r;

    pressureL = i-1 < 0 || type(i-1,j) == SOLID ? pressureC : P(i-1, j);  
    pressureR = i+1 >= gridSize.x || type(i+1,j) == SOLID ? pressureC : P(i+1, j);  
    pressureB = i-1 < 0 || type(i,j-1) == SOLID ? pressureC : P(i, j-1);  
    pressureT = j+1 >= gridSize.y || type(i,j+1) == SOLID ? pressureC : P(i, j+1);  
