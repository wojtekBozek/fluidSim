...
void main()
{
    ...
    if(id.x < gridSize.x + 1 && id.y < gridSize.y)
    {
        ...
        float val = imageLoad(uTex,ivec2(i,j)).r;
        if((i%2 == 0 && j%2 == 1) || (i%2 == 1 && j%2 == 0))
        {            
            if(typeAt(i,j)!= FLUID) return;
            int count = 0;
            float div = texelFetch(divergence,ivec2(i,j),0).r;
            if(typeAt(i-1,j) == SOLID) { imageStore(uTex, ivec2(i,j), vec4(0.0)); return; }  else {count++;}// ścianka dla badanenj prędkości
            // Right
        ...
        {
            if(typeAt(i-1,j)!= FLUID) return;
            float div = texelFetch(divergence,ivec2(i-1,j),0).r;
            int count = 0;
            if(typeAt(i,j) == SOLID) {imageStore(uTex, ivec2(i,j), vec4(0.0)); return; }  else {count++;}// ścianka dla badanenj prędkości
            // Right
            if(typeAt(i-2,j) != SOLID) { count++; } 
            // Bottom
        ...
        imageStore(uTex, ivec2(i,j), vec4(val));
    }
}