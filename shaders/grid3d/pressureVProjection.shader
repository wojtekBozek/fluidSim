...
void main()
{
    ...
    if(id.y < gridSize.y + 1 && id.x < gridSize.x && id.z < gridSize.z)
    {
        ...
        uint bottomType = (j-1 < 0) ? SOLID : texelFetch(cellType, ivec3(i,j-1,k), 0).r;
        uint topType = (j >= gridSize.y) ? SOLID : texelFetch(cellType, ivec3(i,j,k), 0).r;
        ...
        {
            float pressureB = (bottomType == FLUID) ? texelFetch(pressure, ivec3(i, j-1,k), 0).r : 0.0;
            float pressureT = (topType == FLUID) ? texelFetch(pressure, ivec3(i,j,k), 0).r : 0.0;

            float gradPressure = (pressureT - pressureB)/ (dx*dens);
            v -= dt * gradPressure;
        }  
        else
        {
