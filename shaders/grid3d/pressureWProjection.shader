...
void main()
{
    ...
    if(id.x < gridSize.x && id.y < gridSize.y && id.z < gridSize.z+1)
    {
        ...
        uint leftType= (k-1 < 0) ? SOLID : texelFetch(cellType, ivec3(i,j,k-1), 0).r;
        uint rightType = (k >= gridSize.z) ? SOLID : texelFetch(cellType, ivec3(i,j,k), 0).r;
        ...
        {
            float pressureL = (leftType == FLUID) ? texelFetch(pressure, ivec3(i,j,k-1), 0).r : 0.0;
            float pressureR = (rightType == FLUID) ? texelFetch(pressure, ivec3(i,j,k), 0).r : 0.0;
